...
- **Deploy** (opened from the Start panel) — the SRS's Deployment workflow (§5.4):
  - **Compare** — pick a local folder and a remote target path (prefilled with the current desktop folder); every file is classified as Identical, Modified, New (local), or Remote only in a colour-coded table with sizes and modified times, each with a checkbox (Modified/New-local default checked; Remote-only defaults unchecked, since deleting is opt-in). The identical/modified decision is size-only by default (not mtime — nothing preserves timestamps across an upload, so comparing by mtime made byte-identical files show as "Modified" purely from upload-time drift); an opt-in **Verify checksums** checkbox hashes both sides (SHA-256) for same-size files instead, catching the rare case where two different files happen to share a size.
  - **Ignore patterns** — an editable, `.gitignore`-style box (comments, `!` negation, `**` across directories vs. a single `*`/`?` staying within one path segment, trailing `/` for directory-only, `/` in a pattern anchors it to the root) prefilled with the SRS's default set, extendable via an **Import .gitignore** button that reads the local folder's own `.gitignore`. Ignored entries are pruned from both the local walk and the remote SFTP walk — matching directories are never even descended into.
  - **Deploy Selected** — builds a numbered deployment plan (hook commands, what will be backed up/uploaded/deleted with total size, and the configured health check) and shows it before doing anything. A **Dry run** checkbox shows the same plan with nothing executed. Otherwise, confirming runs it in order: pre-deploy hook (a nonzero exit aborts before touching anything) → back up every file about to be overwritten → upload checked files (creating missing remote subdirectories first) → delete checked remote-only files → post-deploy hook → health check — then re-compares automatically.
  - **Pre/post-deploy hooks** — optional single-line remote commands run before and after the file transfer. A failing pre-hook aborts the whole deploy before any backup/upload happens; a failing post-hook is reported as a deploy issue without rolling anything back on its own.
  - **Health checks** — an optional post-deploy check: HTTP (URL + expected status, checked client-side), TCP port reachability, process presence, systemd unit active state, or an arbitrary command's exit code (the last three run server-side over SSH), with configurable retry count/interval. An **Auto-rollback if health check fails** checkbox will automatically restore the backup just taken if the check never passes.
  - **Automatic backup with retention** — before overwriting a file, its current remote content is archived server-side (`tar`) to `<remoteRoot>/.linuxdesk-deploy-backups/<timestamp>.tar.gz`, reusing the same exec plumbing as Compress/Extract. Only files that are about to be modified are backed up — new uploads and remote-only deletions aren't (nothing to restore for those). The last 5 backups per target are kept; recording a 6th prunes the oldest both from local tracking and by deleting its remote archive file.
  - **Rollback to any retained backup** — a Rollback button (enabled once at least one backup exists for the current target) opens a picker listing all retained backups newest-first; restoring overwrites current files with that backup's content, then re-compares.
  - **Production confirmation** — on a profile marked Production, deploying requires typing the exact host to confirm (same friction pattern as recursive folder delete), shown alongside the same deployment plan.

  All of the above — checksums, hooks, all five health check types, backup retention/pruning, and rollback — were verified end-to-end against a real VM: confirming a same-size-different-content file is only caught with checksums on, that a failing pre-hook aborts before any backup, that each health check type correctly reports both healthy and unhealthy states (including spinning up a real local HTTP server for the HTTP check), and that deploying past the 5-backup cap prunes the oldest backup from both the tracking store and the remote filesystem.
- **Audit log** — a local, tamper-evident record (opened from the Start panel) of every connect/disconnect, delete, chmod/chown, service start/stop/restart/enable/disable, process kill, deploy, and rollback LinuxDesk performs, across every host — timestamp, host, remote user, action, outcome (success/failure), and an error excerpt on failure. Each entry's hash covers its own fields plus the previous entry's hash, so a "Verify Integrity" button can detect any edited or deleted line; passwords/passphrases/key material are never logged. Searchable by host/user/action/detail. Stored at `~/.linuxdesk/audit.log`.
...
- The audit log only covers connect/disconnect, delete, chmod/chown, service control, process kill, deploy, and rollback — not every action (e.g. rename, upload/download outside of deploy, compress/extract, edits made in the file editor aren't logged). It's a local file with `Files.readAllLines`-based tamper detection, not a real database — no retention policy/rotation, no CSV/JSON export, no syslog/SIEM forwarding, and command text/exit codes/durations from the SRS's full record shape aren't captured, just action/outcome/detail.
- Deploy has no zero-downtime symlink-swap, and no named deployment profiles or full deployment history (rollback picks from the last 5 retained backups per target, but there's no browsable history of past deployments, no comparing two of them, no repeat, no CSV/JSON export) (`FR-DEP-018, 040–041, 044–048` still ahead). Ignore patterns are a deliberately simple `.gitignore` subset — no character classes (`[abc]`), and `**` doesn't implement every gitignore edge case (e.g. a leading `**/` pattern won't also match at the root with zero directories). Health checks only support one check per deploy, not a chained sequence. Rollback only restores files that were *modified* by the last deploy — it doesn't undo new-file uploads or remote-only deletions from that same deploy, and it's a straight `tar` extraction in place, so unrelated remote changes since that deploy would be overwritten too for the files it touches.
...
src/main/java/com/linuxdesk/
  ...
  ssh/CommandOutcome.java     # output + exit status of an arbitrary remote command (hooks, health checks)
  ...
  deploy/DeployComparer.java  # walks local + remote trees, classifies every file (size or opt-in checksum)
  deploy/IgnorePatterns.java  # .gitignore-subset matcher (wildcards incl. **, negation, dir-only, anchored)
  deploy/HealthChecker.java   # HTTP/TCP/process/systemd-unit/command post-deploy health checks with retry
  deploy/DeployBackupRecord.java # one retained backup: host, remoteRoot, backupPath, timestamp
  deploy/DeployBackupStore.java  # persists up to 5 retained backups per (host, remoteRoot) target, newest first
