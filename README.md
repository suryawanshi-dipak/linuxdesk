...
- **Deploy** (opened from the Start panel) — the SRS's Deployment workflow (§5.4):
  - **Compare** — pick a local folder and a remote target path (prefilled with the current desktop folder); every file is classified as Identical, Modified, New (local), or Remote only in a colour-coded table with sizes and modified times, each with a checkbox (Modified/New-local default checked; Remote-only defaults unchecked, since deleting is opt-in). The identical/modified decision is size-only (not mtime) — nothing preserves timestamps across an upload, so comparing by mtime made byte-identical files show as "Modified" purely from upload-time drift; this is a deliberate, documented simplification pending real content comparison later.
  - **Ignore patterns** — an editable, `.gitignore`-style box (comments, `*`/`?` wildcards, trailing `/` for directory-only, `/` in a pattern anchors it to the root) prefilled with the SRS's default set, extendable via an **Import .gitignore** button that reads the local folder's own `.gitignore`. Ignored entries are pruned from both the local walk and the remote SFTP walk — matching directories are never even descended into.
  - **Deploy Selected** — builds a numbered deployment plan (what will be backed up/uploaded/deleted, with total size) and shows it before doing anything. A **Dry run** checkbox shows the same plan with nothing executed. Otherwise, confirming runs it: back up every file about to be overwritten, upload checked files (creating missing remote subdirectories first), delete checked remote-only files, then re-compare automatically.
  - **Automatic backup** — before overwriting a file, its current remote content is archived server-side (`tar`) to `<remoteRoot>/.linuxdesk-deploy-backups/<timestamp>.tar.gz`, reusing the same exec plumbing as Compress/Extract. Only files that are about to be modified are backed up — new uploads and remote-only deletions aren't (nothing to restore for those).
  - **One-click rollback** — a Rollback button (enabled once a backup exists for the current target) restores the most recent backup, overwriting current files with the pre-deploy content, then re-compares. Only the immediately-previous deployment is kept, not a full history.
  ...
  All of the above — backup, deploy, rollback, and the size-only comparison — were verified end-to-end against a real VM, including confirming that a rollback restores byte-for-byte the exact pre-deploy content.
...
- Deploy has no pre/post-deploy hooks, no health checks, no zero-downtime symlink-swap, no backup retention/pruning, and no named deployment profiles/history — rollback only keeps the single most recent backup per target, not a browsable history (`FR-DEP-016–018, 032, 034, 041, 044–060` mostly still ahead). Ignore patterns are a deliberately simple `.gitignore` subset — no negation (`!`), no `**`, no character classes. Comparison is size-only, so two different files that happen to share a size would be misclassified as identical — no checksum fallback yet (`FR-DEP-003`). Rollback only restores files that were *modified* by the last deploy — it doesn't undo new-file uploads or remote-only deletions from that same deploy.
...
src/main/java/com/linuxdesk/
  ...
  deploy/DeployBackupRecord.java # one target's most recent backup: host, remoteRoot, backupPath, timestamp
  deploy/DeployBackupStore.java  # persists the single latest backup per (host, remoteRoot) target
